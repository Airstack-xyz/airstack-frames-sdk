...
import { getFarcasterUserNFTBalances } from "../functions/getFarcasterUserNFTBalances";
import { config } from "dotenv";

config();

describe("getFarcasterUserNFTBalances", () => {
  it("should fail if no api key is porivded", async () => {
    const { data, error } = await getFarcasterUserNFTBalances({ fid: 3 });
