...
import { getFarcasterFollowers } from "../functions/getFarcasterFollowers";
import { config } from "dotenv";

config();

describe("getFarcasterFollowers", () => {
  ...
  it("should return the followers lengths that has less than or equal to the set limit", async () => {
    init(process.env.AIRSTACK_API_KEY ?? "");
    const limit = 10;
    const { data, error } = await getFarcasterFollowers({ fid: 1, limit });
    expect(error).toBeNull();
  ...
  it("should paginate data correctly by data comparison", async () => {
    init(process.env.AIRSTACK_API_KEY ?? "");
    const { data } = await getFarcasterFollowers({
      fid: 1,
      limit: 2,
    });
    expect(data).toBeTruthy();
